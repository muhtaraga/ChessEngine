...
std::vector<int*> flat_param_pointers(EvalParams& p) {
    ...
    // 6d) geçer piyon blokajı (tune edilir)
    v.push_back(&p.blockade_mg); v.push_back(&p.blockade_eg);
...
const std::vector<std::string>& flat_param_names() {
    static const std::vector<std::string> names = [] {
        ...
        n.push_back("blockade_mg"); n.push_back("blockade_eg");
...
int eval_frozen_start() {
    ...
    // bishop_pair(2) + rook_file(4) + threats(8) + outpost(4) + blockade(2) = 826.
    // King safety bundan sonra başlar.
