...
// --- Geçer piyon blokajı testleri (passer_blockade yardımcısıyla) ---

// Beyaz d5 geçer piyonunun durak karesi d6'da siyah at var -> beyaz cezalanır.
// İkinci tahta: at d6'dan g6'ya çekilince blokaj kalkar -> katkı 0. Aynı piyon
// dizilimi, yalnız blokaj koşulu düşer -> boş geçmez.
TEST(Eval, BlockadePenalizesStoppedPasser) {
    Board blocked;
    ASSERT_TRUE(blocked.set_fen("4k3/8/3n4/3P4/8/8/8/4K3 w - - 0 1"));
    int mg = 0, eg = 0;
    passer_blockade(blocked, mg, eg);
    EXPECT_EQ(mg, -BlockadeMg);
    EXPECT_EQ(eg, -BlockadeEg);

    Board free_pawn;  // at g6'da: d5 hâlâ geçer ama durak karesi boş
    ASSERT_TRUE(free_pawn.set_fen("4k3/8/6n1/3P4/8/8/8/4K3 w - - 0 1"));
    int mg2 = 0, eg2 = 0;
    passer_blockade(free_pawn, mg2, eg2);
    EXPECT_EQ(mg2, 0);
    EXPECT_EQ(eg2, 0);
}

// Blokaj YALNIZ geçer piyona uygulanır: siyah e6 piyonu d5'i geçer olmaktan çıkarır
// (PassedMask komşu sütunun önünü de içerir) -> at d6 dursa bile ceza YOK.
// Ayrıca: rakip PİYON asla blokajcı olamaz, çünkü durak karesindeki piyon zaten
// geçerliği bozar (bu testin ikinci tahtası tam da onu gösterir).
TEST(Eval, BlockadeOnlyAppliesToPassers) {
    Board not_passed;
    ASSERT_TRUE(not_passed.set_fen("4k3/8/3np3/3P4/8/8/8/4K3 w - - 0 1"));
    int mg = 0, eg = 0;
    passer_blockade(not_passed, mg, eg);
    EXPECT_EQ(mg, 0);
    EXPECT_EQ(eg, 0);

    Board pawn_in_front;  // siyah piyon d6: d5 geçer DEĞİL -> blokaj terimi hiç girmez
    ASSERT_TRUE(pawn_in_front.set_fen("4k3/8/3p4/3P4/8/8/8/4K3 w - - 0 1"));
    int mg2 = 0, eg2 = 0;
    passer_blockade(pawn_in_front, mg2, eg2);
    EXPECT_EQ(mg2, 0);
    EXPECT_EQ(eg2, 0);
}

// Kendi taşımız önümüzdeyse blokaj DEĞİL (onu çekebiliriz) -> ceza yok.
// (Şah a8'de: beyaz Nd6 e8'e çek verirdi, sıra beyazdayken illegal olurdu.)
TEST(Eval, BlockadeIgnoresOwnPieceInFront) {
    Board b;
    ASSERT_TRUE(b.set_fen("k7/8/3N4/3P4/8/8/8/4K3 w - - 0 1"));  // BEYAZ at d6
    int mg = 0, eg = 0;
    passer_blockade(b, mg, eg);
    EXPECT_EQ(mg, 0);
    EXPECT_EQ(eg, 0);
}

// Renk simetrisi: beyaz Pd5+Nd3, siyah Pd4+Nd6 — tam dikey ayna (d5^56==d4,
// d3^56==d6, e1^56==e8). Beyazın d5'i Nd6 ile, siyahın d4'ü Nd3 ile bloke ->
// katkı tam sıfır, evaluate() de 0.
TEST(Eval, BlockadeSymmetry) {
    Board b;
    ASSERT_TRUE(b.set_fen("4k3/8/3n4/3P4/3p4/3N4/8/4K3 w - - 0 1"));
    int mg = 0, eg = 0;
    passer_blockade(b, mg, eg);
    EXPECT_EQ(mg, 0);
    EXPECT_EQ(eg, 0);
