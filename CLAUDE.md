...
- [ ] **History-tabanlı LMR** (birleşik main+cont stat ile indirim ayarı): SIRADAKİ İŞ.
      Kalıcılık öncesi denendi -> stat ölçeği çok küçük olduğundan no-op; bölen
      kalıcılık sonrası YENİDEN ÖLÇÜLECEK. Simetrik form, ±2 ply kelepçe.
...
tablo kalıcılığı paketi SPRT +31.6 ± 13.8 Elo, LLR 2.96 tam kabul** (yeni baseline
`ae20258`). Countermove (regresyon) denendi, geri alındı. **SIRADAKİ İŞ: history-tabanlı
LMR** (birleşik main+cont stat; bölen kalıcılık sonrası yeniden ölçülecek), sonra
Faz 2D (Lazy SMP). Ayrıntılı adım-adım kayıt ve en güncel özet aşağıdaki bölümlerde +
memory `proje-durumu`.
...
+31.6 Elo TAM KABUL (yeni baseline ae20258)**; countermove + main-history-LMR denendi,
geri alındı. SIRADAKİ: history-tabanlı LMR (bölen yeniden ölçülecek) -> sonra Faz 2D (Lazy
