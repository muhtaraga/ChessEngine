...
- [ ] **Continuation history** (bir önceki hamle bağlamıyla history): sıralamayı VE
      history-tabanlı LMR'yi güçlendirir. **SIRADAKİ İŞ (yeni oturum).** Not:
      history-tabanlı LMR yalnız main history ile denendi -> no-op / over-reduction
      (malus ile örtüşme) -> geri alındı; continuation history bunu çözecek.
...
**Güncel durum (2026-07-09): FAZ 1 + FAZ 2A + FAZ 2B TAMAM, FAZ 2C büyük ölçüde bitti.**
Motor UCI üzerinden GUI'ye bağlanıyor, legal oynuyor, perft geçiyor. Toplam 100
...
güçlendirme: **history malus (işaretli history) SPRT +22.2 Elo kabul** (yeni baseline).
Countermove (regresyon) + main-history-tabanlı LMR (no-op/over-reduction) denendi,
geri alındı. **SIRADAKİ İŞ (yeni oturum): continuation history** (sıralamayı + gerçek
history-LMR'yi güçlendirir), sonra Faz 2D (Lazy SMP). Ayrıntılı adım-adım kayıt ve en
güncel özet aşağıdaki bölümlerde + memory `proje-durumu`.
...
- Move ordering güçlendirme (Adım 8 sonrası pivot):
  ...
  - Main-history-tabanlı LMR (commit 4520bc1) — NO-OP (ödül-only) sonra malus
    ...
    aşırı-indirim. Kök sebep: yalnız main history var, malus ile örtüşüyor.
    **ÇÖZÜM = continuation history (SIRADAKİ İŞ, yeni oturum): bir önceki hamle
    bağlamıyla history -> hem sıralama hem gerçek history-LMR güçlenir.**
...
malus SPRT +22.2 Elo KABUL (yeni baseline); countermove + main-history-LMR denendi,
geri alındı. SIRADAKİ: continuation history (yeni oturum) -> sonra Faz 2D (Lazy
